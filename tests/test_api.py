...
class APITestCase(unittest.TestCase):
    ...
    def test_unauthorized(self):
        ...
        with app.test_request_context("/foo"):
            response = api.unauthorized(response)
        self.assertEqual(
            response.headers["WWW-Authenticate"], 'Basic realm="flask-restful"'
        )
    ...
    def test_handle_error_401_sends_challege_default_realm(self):
        ...
        with app.test_request_context("/foo"):
            ...
            self.assertEqual(
                resp.headers["WWW-Authenticate"], 'Basic realm="flask-restful"'
            )
    ...
    def test_handle_error_401_sends_challege_configured_realm(self):
        ...
        with app.test_request_context("/foo"):
            ...
            self.assertEqual(resp.status_code, 401)
            self.assertEqual(
                resp.headers["WWW-Authenticate"], 'Basic realm="test-realm"'
            )
        ...
        fields = OrderedDict(
            [
                ...
                (
                    "fee",
                    flask_restful.fields.List(
                        flask_restful.fields.List(flask_restful.fields.String)
                    ),
                ),
            ]
        )
    ...
    def test_handle_server_error(self):
        ...
        with app.test_request_context("/foo"):
            ...
            self.assertEqual(
                resp.data.decode(), dumps({"message": "Internal Server Error"}) + "\n"
            )
    ...
    def test_media_types_method(self):
        ...
        with app.test_request_context(
            ...
            self.assertEqual(
                api.mediatypes_method()(Mock()), ["application/xml", "application/json"]
            )
    ...
    def test_media_types_q(self):
        ...
        with app.test_request_context(
            ...
            self.assertEqual(api.mediatypes(), ["application/json", "application/xml"])
    ...
    def test_add_two_conflicting_resources_on_same_endpoint(self):
        ...
        self.assertRaises(
            ValueError, api.add_resource, Foo2, "/foo/toto", endpoint="bar"
        )
    ...
    def test_fr_405(self):
        ...
        self.assertEqual(allow, {"HEAD", "OPTIONS"}.union(HelloWorld.methods))
    ...
    def test_exception_header_forwarded(self):
        ...
        with app.test_client() as client:
            foo = client.get("/foo")
            self.assertEqual(foo.get_etag(), unquote_etag(quote_etag("myETag")))
    ...
    def test_read_json_settings_from_config(self):
        class TestConfig():
            RESTFUL_JSON = {"indent": 2, "sort_keys": True, "separators": (", ", ": ")}
