...
class ReqParseTestCase(unittest.TestCase):
    ...
    def test_help_with_unicode_error_msg(self, abort):
        ...
        with app.app_context():
            ...
            req = Mock(["values"])
            req.values = MultiDict([("foo", "\xf0\x9f\x8d\x95")])
            parser.parse_args(req)
            expected = {"foo": "Bad choice: \xf0\x9f\x8d\x95 is not a valid choice"}
            abort.assert_called_with(400, message=expected)
    ...
    def test_help_no_error_msg(self, abort):
        ...
        with app.app_context():
            parser = RequestParser()
            parser.add_argument(
                "foo", choices=["one", "two"], help="Please select a valid choice"
            )
            req = Mock(["values"])
            req.values = MultiDict([("foo", "three")])
            parser.parse_args(req)
    ...
    def test_parse_foo_operators_four_hunderd(self):
        ...
        with app.app_context():
            ...
            self.assertRaises(
                ...
                lambda: parser.parse_args(Request.from_values("/bubble?foo=bar")),
            )
    ...
    def test_parse_required(self):
        ...
        with app.app_context():
            ...
            self.assertEqual(
                ...
                (
                    {
                        "foo": "Missing required parameter in "
            ...
            self.assertEqual(
                ...
                (
                    {
                        "bar": "Missing required parameter in "
                        "the post body or the query "
    ...
    def test_parse_error_bundling(self):
        ...
        with app.app_context():
            ...
            try:
                ...
                message = e.data["message"]
            error_message = {
                "foo": "Missing required parameter in the post "
    ...
    def test_namespace_configurability(self):
        ...
        self.assertTrue(
            isinstance(RequestParser(namespace_class=dict).parse_args(req), dict)
        )
