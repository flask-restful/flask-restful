...
class FieldsTestCase(unittest.TestCase):
    def test_decimal_trash(self):
        self.assertRaises(
            MarshallingException, lambda: fields.Float().output("a", {"a": "Foo"})
        )
    ...
    def test_int_decode_error(self):
        ...
        self.assertRaises(
            ...
            lambda: field.output(
                "hey", {"hey": "Explode please I am nowhere looking like an int"}
            ),
        )
    ...
    def test_float_decode_error(self):
        field = fields.Float()
        self.assertRaises(
            MarshallingException, lambda: field.output("hey", {"hey": "Explode!"})
        )

    PI_STR = "3."
    PI = Decimal(PI_STR)
    ...
    def test_infinite_fixed(self):
        field = fields.Fixed()
        self.assertRaises(
            MarshallingException, lambda: field.output("hey", {"hey": "+inf"})
        )
        self.assertRaises(
            MarshallingException, lambda: field.output("hey", {"hey": "-inf"})
        )
    ...
    def test_advanced_fixed(self):
        field = fields.Fixed()
        self.assertRaises(
            MarshallingException, lambda: field.output("hey", {"hey": "NaN"})
        )
    ...
    def test_list_of_nested(self):
        ...
        self.assertEqual(
            [OrderedDict([("a", 1)]), OrderedDict([("a", 2)]), OrderedDict([("a", 3)])],
